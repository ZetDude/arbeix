...
export default function Entries(props: Props) {
  const { indent, dedent, entries } = props;
    ...
    <div>
      <div className={`${style.form} ${style.sidebyside}`}>
        <div className={`${style.left} ${style.sidebyside}`}>
          {isPath && (
            <i
              className={`fa fa-angle-double-left fa-2x ${style.icon}`}
              onClick={dedent}
            />
          )}
          {!isNotFinal && entries.final !== undefined && (
            <span>
              {`Valitud toode: ${entries.final[0]} (${entries.final[1]})`}
            </span>
          )}
          {isNotFinal && (
            <Searchable
